...
// ======================================================
_SceneVirtualWorld::_SceneVirtualWorld()
{
    //ctor
...
GLint _SceneVirtualWorld::initGL()
{
    glewInit();
    ...
    myMusic->iniSounds();
    Sleep(250);
    myMusic->playMusic(_SceneVirtualWorld::song);

    skyBox->boxInit();
    skyBox->myTex[0].loadTexture("images/virtualScene/back.png");
    skyBox->myTex[1].loadTexture("images/virtualScene/back.png");
    skyBox->myTex[2].loadTexture("images/virtualScene/back.png");
    skyBox->myTex[3].loadTexture("images/virtualScene/back.png");
    skyBox->myTex[4].loadTexture("images/virtualScene/back.png");
    skyBox->myTex[5].loadTexture("images/virtualScene/back.png");
    Sleep(250);
    ...
    box->modelInit("images/virtualScene/crate.png");
    Sleep(250);
    // Road
    for(int i = 0; i < _SceneVirtualWorld::numRoadChunks; i++){
        road[i] = new _ground();
        //DEBUG FOR VISUALIZATION
        if(i%2 == 0){
            road[i]->modelInit("images/virtualScene/virtual_road.jpg");
        } else {
            road[i]->modelInit("images/virtualScene/virtual_road.jpg");
...
void _SceneVirtualWorld::reSize(GLint width, GLint height)
{
...
float _SceneVirtualWorld::deltaT = 0;

void _SceneVirtualWorld::drawScene()
{
    // --------------------------------------
    ...
    _SceneVirtualWorld::deltaT = elapsed.count();
    ...
    roadChunk(_SceneVirtualWorld::deltaT);
...
void _SceneVirtualWorld::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch(uMsg)
    {
    case WM_KEYDOWN:
...
void _SceneVirtualWorld::resetGame()
{
    playerLives = 5;
    coinCount = 0;
    gameOver = false;
...
void _SceneVirtualWorld::roadChunk(float deltaT)
{
    float dz = _SceneVirtualWorld::speed * deltaT;

    for (int i = 0; i < numRoadChunks; i++) {
        road[i]->moveZ(dz);

        if (road[i]->pos.z > _SceneVirtualWorld::recyclePoint) {
            road[i]->moveZ(-_SceneVirtualWorld::totalLoopLength);
...
void _SceneVirtualWorld::drawObstacles(int chunkIndex)
{
...
void _SceneVirtualWorld::drawCoins(int chunkIndex)
{
    float coinY = -0.2f;
...
void _SceneVirtualWorld::generateChunkObstacles(int chunkIndex)
{
...
void _SceneVirtualWorld::clearChunkObstacles(int chunkIndex)
{
    for (int row = 0; row < ROWS_PER_CHUNK; row++) {
        for (int lane = 0; lane < LANES; lane++) {
...
void _SceneVirtualWorld::drawTestObstacles(int chunkIndex)
{
...
void _SceneVirtualWorld::mouseMapping(int x, int y)
{
...
void _SceneVirtualWorld::drawLivesHUD()
{
...
void _SceneVirtualWorld::drawCoinHUD()
{
...
void _SceneVirtualWorld::drawGameOverHUD()
{
