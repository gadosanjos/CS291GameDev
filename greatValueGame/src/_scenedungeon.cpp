...
GLint _SceneDungeon::initGL()
{
    ...
    // skybox
    skyBox->boxInit();
    skyBox->myTex[0].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Back.png");
    skyBox->myTex[1].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Front.png");
    skyBox->myTex[2].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Top.png");
    skyBox->myTex[3].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Bottom.png");
    skyBox->myTex[4].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Left.png");
    skyBox->myTex[5].loadTexture("images/ProjectSkyboxes/Dungeon_Box_Right.png");
    // HUD
    ...
    gameWinPanel->initIcon("images/transition/lvl3complete.png");
    ...
    // player model
    player->init("models/doomguy/tris.md2", "models/doomguy/lightblue.pcx", "models/doomguy/w_rlauncher.md2", "models/doomguy/w_rlauncher.pcx");
    player->pos.x = 0.0f;
    player->pos.y = 0.0f;
    player->pos.z = 0.0f;
    player->yaw = 0.0f;
    ...
    // world ground
    worldGround->modelInit("images/dungeontexture.png");
...
void _SceneDungeon::reSize(GLint width, GLint height)
{
    wWidth = (float)width;
...
float _SceneDungeon::deltaT = 0.0f;

void _SceneDungeon::drawScene()
{
    // --------------------------------------
    ...
    _SceneDungeon::deltaT = elapsed.count();
...
void _SceneDungeon::checkEnemyPlayerDamage(float deltaT){
...
// Mob helpers
// ======================================================
void _SceneDungeon::randomizeEnemySpawnPositions(vec3 playerPos, int distanceRange, int minDistance, float timeRemaining)
{
...
void _SceneDungeon::spawnPortals(float x, float y, float z, int portalCollor)
{
    PortalEffect p;
...
void _SceneDungeon::spawnEnemy(float x, float y, float z, int mobSelection)
{
    _Enemy* e = new _Enemy();
    switch(mobSelection){
        case dragonknight:
...
void _SceneDungeon::updatePortals(float deltaT)
{
...
void _SceneDungeon::updateEnemies(float deltaT)
{
    for (size_t i = 0; i < enemies.size(); i++) {
...
void _SceneDungeon::drawPortals()
{
...
void _SceneDungeon::drawEnemies()
{
    for (size_t i = 0; i < enemies.size(); i++) {
        enemies[i]->draw();
    }
...
void _SceneDungeon::separateEnemies()
{
    for (size_t i = 0; i < enemies.size(); i++) {
...
void _SceneDungeon::clearEnemies()
{
    for (size_t i = 0; i < enemies.size(); i++) {
        delete enemies[i];
    }
...
void _SceneDungeon::clearPortals()
{
    for (size_t i = 0; i < portals.size(); i++) {
        delete portals[i].sprite;
...
void _SceneDungeon::removeDeadEnemies()
{
...
void _SceneDungeon::initMagicBullets()
{
...
_Enemy* _SceneDungeon::findNearestEnemyInRange(float range)
{
...
void _SceneDungeon::updateAutoMagic(float deltaT)
{
...
void _SceneDungeon::updateMagicBullets(float deltaT)
{
    for (int i = 0; i < MAX_MAGIC_BULLETS; i++) {
...
void _SceneDungeon::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_KEYDOWN:
...
void _SceneDungeon::drawLivesHUD()
{
...
void _SceneDungeon::drawGameOverHUD()
{
...
void _SceneDungeon::drawGameWinHUD()
{
...
void _SceneDungeon::drawTimerHUD()
{
...
void _SceneDungeon::drawText(float x, float y, const char* text, float scale)
{
    glMatrixMode(GL_PROJECTION);
    glPushMatrix();
    glLoadIdentity();
